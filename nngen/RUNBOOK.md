...
## 8. БЕЙК ассетов карты (основной режим: один раз сгенерить всё)

Архитектура «asset bake»: нейронка работает офлайн-фабрикой, игра живёт на
статике. Никаких туннелей и рантайм-генерации.

```bash
# любая машина (или уже закоммичено):
python nngen/bake_map.py prepare      # 85 блокаутов из procgen

# GPU-машина (~1 час на 85 ассетов):
python nngen/bake_map.py generate     # img2img каждого блокаута
                                      # прерывать безопасно — продолжит с места

# любая машина:
python nngen/bake_map.py finish       # вырезка по маске, палитра, спрайты
```

Результат: `nngen/bake/out/<класс>/*.png` + `manifest.json` (якоря) —
готово к заливке в ассеты игры. Коммитить `nngen/bake/raw/` (сырые генерации)
и `out/`.

Тюнинг: strength на класс задаётся в `registry()` bake_map.py
(дома 0.55, деревья 0.6, пропсы 0.5, тайлы 0.4). Больше strength = больше
деталей нейронки, меньше = ближе к procgen-геометрии.
